...
final class FileStore: ObservableObject {
    ...
    init(logManager: Logging) {
        ...
        logManager.debugSync("FileStore initialized", category: "FileStore")
    }
    ...
    func setFiles(_ newFiles: [File]) {
        logManager.debugSync("Setting files", category: "FileStore", metadata: ["count": "\(newFiles.count)"])
        files = newFiles
    }

    /// Appends files to the existing files array (used for batched UI updates)
    func appendFiles(_ newFiles: [File]) {
        logManager.debugSync("Appending files", category: "FileStore", metadata: ["count": "\(newFiles.count)", "totalAfter": "\(files.count + newFiles.count)"])
    ...
    func updateFile(_ updatedFile: File) {
        if let index = files.firstIndex(where: { $0.id == updatedFile.id }) {
            ...
            logManager.debugSync("Updated file", category: "FileStore", metadata: [
                "id": updatedFile.id,
                "status": updatedFile.status.rawValue
            ])
        } else {
            logManager.errorSync("Attempted to update non-existent file", category: "FileStore", metadata: ["id": updatedFile.id])
        }
    }
    ...
    func clearFiles() {
        logManager.debugSync("Clearing all files", category: "FileStore")
        files.removeAll()
    }
