...
/// Results are written directly to the injected FileStore.
@MainActor
class RecalculationManager: ObservableObject {
    ...
    /// Centralized file state — results are written here directly.
    ...
    init(logManager: Logging, fileProcessorService: FileProcessorService, settingsStore: SettingsStore, fileStore: FileStore) {
        self.logManager = logManager
        self.fileProcessorService = fileProcessorService
        self.settingsStore = settingsStore
        self.fileStore = fileStore

        logManager.debugSync("RecalculationManager initialized", category: "RecalculationManager")
    }
    ...
    func startRecalculation(
        ...
        logManager.debugSync("startRecalculation called", category: "RecalculationManager", metadata: ["newDestination": newDestinationURL?.path ?? "nil"])
        ...
        guard !currentFiles.isEmpty else {
            logManager.debugSync("No files to recalculate, resetting state", category: "RecalculationManager")
        ...
        // 3. Update UI state to indicate recalculation is in progress.
        self.isRecalculating = true
        self.error = nil // Clear any previous errors

        // 4. Create and start a new asynchronous task for the recalculation.
        currentRecalculationTask = Task {
            do {
                ...
                self.fileStore.setFiles(recalculatedFiles)
                self.isRecalculating = false
                self.error = nil
                self.didFinish.send()
    ...
    func cancelRecalculation() {
        logManager.debugSync("cancelRecalculation called", category: "RecalculationManager")
        currentRecalculationTask?.cancel()
