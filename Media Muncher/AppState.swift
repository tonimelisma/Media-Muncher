...
class AppState: ObservableObject {
    ...
    init(
        ...
        logManager.infoSync("AppState.init() started", category: "AppState")

        // Subscribe to volume changes
        logManager.debugSync("Subscribing to volumeManager.$volumes", category: "AppState")
        volumeManager.$volumes
            .receive(on: DispatchQueue.main)
            .sink { [weak self] newVolumes in
                self?.logManager.debugSync("Received volume changes from publisher", category: "AppState", metadata: ["count": "\(newVolumes.count)"])
        ...
        logManager.debugSync("Subscribing to self.$selectedVolumeID", category: "AppState")
        self.$selectedVolumeID
            .receive(on: DispatchQueue.main)
            .sink { [weak self] volumeID in
                self?.logManager.debugSync("Received selectedVolumeID change from publisher", category: "AppState", metadata: ["volumeID": volumeID ?? "nil"])
                self?.startScan(for: volumeID)
            }
            .store(in: &cancellables)

        // Subscribe to destination changes and setup recalculation chain
        logManager.debugSync("Setting up destination change handling", category: "AppState")
        setupDestinationChangeHandling()
        setupRecalculationManagerBindings()

        // Initial state
        logManager.debugSync("Setting initial volumes from volumeManager", category: "AppState")
        self.volumes = volumeManager.volumes
        logManager.debugSync("Initial volumes set", category: "AppState", metadata: ["count": "\(self.volumes.count)"])
        logManager.infoSync("AppState initialized successfully", category: "AppState")
        ensureVolumeSelection()
    }

    func ensureVolumeSelection() {
        logManager.debugSync("ensureVolumeSelection called", category: "AppState")
        ...
        if !currentSelectionIsValid {
            ...
            if let firstVolume = self.volumes.first {
                logManager.debugSync("Selecting first volume", category: "AppState", metadata: ["name": firstVolume.name, "id": firstVolume.id])
                self.selectedVolumeID = firstVolume.id
            } else {
                // If no volumes are available, clear the selection.
                logManager.debugSync("No volumes available to select, clearing selection", category: "AppState")
    ...
    private func startScan(for volumeID: Volume.ID?) {
        logManager.debugSync("startScan called", category: "AppState", metadata: ["volumeID": volumeID ?? "nil"])
        ...
        guard let volumeID = volumeID,
            ...
            logManager.debugSync("No volume ID provided or volume not found, skipping scan", category: "AppState")
            return
        }

        let url = URL(fileURLWithPath: volume.devicePath, isDirectory: true)
        logManager.debugSync("Starting scan for URL", category: "AppState", metadata: ["path": url.path])
        ...
        self.scanTask = Task {
            ...
            await MainActor.run {
                self.state = .idle
                self.logManager.debugSync("Updated UI to idle state", category: "AppState", metadata: ["finalCount": "\(self.fileStore.files.count)"])
            }
        }
    }

    func cancelScan() {
        logManager.debugSync("cancelScan called", category: "AppState")
        scanTask?.cancel()
        self.state = .idle
    }

    func cancelImport() {
        logManager.debugSync("cancelImport called", category: "AppState")
        importTask?.cancel()
    }
            ...
            do {
                ...
                if settingsStore.settingAutoEject,
                   ...
                   let volumeToEject = volumes.first(where: { $0.id == selectedVolumeID }) {
                    do {
                        try volumeManager.ejectVolume(volumeToEject)
                    } catch {
                        await MainActor.run {
                            self.error = .ejectFailed(volumeName: volumeToEject.name, reason: error.localizedDescription)
                        }
                    }
                }
